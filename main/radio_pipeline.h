 ...
 * undocumented), an AVRCP/console/companion-display next/previous/select
 * command arrived (avrcp_uart.h, console_cli.h, companion_server.h), or a
 * console `cal` command requested LED/audio latency calibration
 * (console_cli.h, latency_cal.h).
 *
 * Returns ESP_OK and sets *out_station_cmd to AVRCP_CMD_NEXT/AVRCP_CMD_PREV/
 * AVRCP_CMD_SELECT when the caller should switch stations - the session was
 * still healthy, so this is NOT a failure and must not attract the caller's
 * error backoff. When *out_station_cmd is set to AVRCP_CMD_SELECT,
 * *out_select_index is also filled in with which RADIO_STATIONS index to
 * switch to (companion_server.h's `POST /tune` - the only source of this
 * command today); left untouched for AVRCP_CMD_NEXT/PREV/NONE, so a caller
 * that only cares about NEXT/PREV can safely ignore it. Returns ESP_OK and
 * sets *out_calibrate to true when the caller should run latency_cal_run()
 * instead of restarting playback - also not a failure, and mutually
 * exclusive with *out_station_cmd (at most one of the two is ever set on a
 * given call; both stay at their "nothing happened" default otherwise).
 * Returns ESP_OK with both left at their defaults on a normal refresh
 * (max_session_ticks elapsed, or a clean HLS live-window restart that could
 * not be done in place). Returns the element's error otherwise. Any
 * out-parameter may be NULL if the caller does not care.
 *
